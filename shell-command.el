...
;;;; Added by mjc.
(defcustom async-shell-command-prompt
  "Async Shell command [%w]%$ "
  "*The prompt string for `shell-command' when tab-completion is enabled.
Some %-sequences are available to customize this variable.  For more
detail, see the document of `shell-command-make-prompt-string'."
  :type 'string
...
(let (current-load-list)
  (defadvice async-shell-command
    (before async-shell-command-with-completion disable compile)
    "Defined in shell-command.el, to enable tab-completion of commands
and dir/filenames within the input context.  Its prompt string is kept
by `async-shell-command-prompt'."
    (interactive
     (lexical-let ((command (shell-command-read-minibuffer async-shell-command-prompt
				     default-directory
				     nil nil nil 'shell-command-history)))
       (list command
	     (or
	      current-prefix-arg
	      (get-buffer-create (concat "*Async: " command "*"))))))))
...
`grep', `grep-find' and `compile'."
  (let ((commands
	 '(shell-command async-shell-command shell-command-on-region grep grep-find compile)))
    (while commands
